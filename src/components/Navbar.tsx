  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#D1E2D3] border-b border-[#C2D4C4] px-4 sm:px-8 lg:px-12 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        {/* LOGO - Utilizando fonte serifada elegante idêntica ao PDF */}
        ...
        {/* LINKS DE NAVEGAÇÃO - Perfeitamente centralizados e com pesos idênticos */}
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={`text-sm font-semibold tracking-wide transition-colors duration-200 hover:text-[#1F3A1F] ${
                isActive(link.path) ? "text-[#2A4B2A]" : "text-[#2A4B2A]/90"
              }`}
            >
              {link.name}
            </Link>
        ...
        {/* ÍCONES DE AÇÃO + BOTÃO ENTRAR (PÍLULA) */}
        <div className="hidden md:flex items-center gap-6">
          ...
          <button className="px-9 py-2.5 rounded-full bg-[#2A4B2A] hover:bg-[#1F3A1F] text-white font-medium text-sm transition-all duration-200 shadow-sm">
            Entrar
          </button>
        </div>
      ...
      {/* MENU RESPONSIVO EXPANSÍVEL (MOBILE) */}
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-[#D1E2D3] border-t border-[#C2D4C4] p-6 shadow-xl flex flex-col gap-5 animate-fadeIn">
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <button className="px-8 py-2.5 rounded-full bg-[#2A4B2A] text-white font-medium text-sm">
              Entrar
            </button>
          </div>
        </div>
